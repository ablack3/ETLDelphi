...
-- Deduplicate by drug_exposure_id in case joins (e.g. map_visit) multiply rows.
...
rows AS (
    SELECT
        ...
        REPLACE(SUBSTR(COALESCE(NULLIF(TRIM(o.drug_ndc_normalized), ''), o.drug_name), 1, 50), '*', '') AS drug_source_value,
        COALESCE(d.drug_source_concept_id, 0) AS drug_source_concept_id,
        SUBSTR(o.route, 1, 50) AS route_source_value,
        SUBSTR(o.dose_units, 1, 50) AS dose_unit_source_value,
        ROW_NUMBER() OVER (PARTITION BY o.drug_exposure_id ORDER BY o.order_id, o.encounter_id) AS rn
    FROM ord o
    ...
    LEFT JOIN stg.custom_concept_mapping cust ON REPLACE(TRIM(cust.source_value), '*', '') = REPLACE(TRIM(SUBSTR(COALESCE(NULLIF(TRIM(o.drug_ndc_normalized), ''), o.drug_name), 1, 50)), '*', '') AND cust.domain = 'drug'
