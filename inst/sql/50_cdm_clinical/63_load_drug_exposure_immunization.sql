...
SELECT
    ...
    REPLACE(SUBSTR(COALESCE(i.vaccine_name, i.vaccine_cvx), 1, 50), '*', ''),
...
LEFT JOIN stg.custom_concept_mapping cust ON REPLACE(TRIM(cust.source_value), '*', '') = REPLACE(TRIM(SUBSTR(COALESCE(i.vaccine_name, i.vaccine_cvx), 1, 50)), '*', '') AND cust.domain = 'drug'
