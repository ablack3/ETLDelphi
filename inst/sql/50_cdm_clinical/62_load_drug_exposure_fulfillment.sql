...
mapped AS (
    ...
    LEFT JOIN stg.custom_concept_mapping cust ON REPLACE(TRIM(cust.source_value), '*', '') = REPLACE(TRIM(SUBSTR(COALESCE(NULLIF(TRIM(f.drug_ndc_normalized), ''), f.drug_name), 1, 50)), '*', '') AND cust.domain = 'drug'
),
rows AS (
    SELECT
        m.drug_exposure_id,
        mp.person_id,
        COALESCE(NULLIF(m.drug_concept_id, 0), m.cust_concept_id, 0) AS drug_concept_id,
        m.dispense_date,
        CASE WHEN m.days_of_supply IS NOT NULL AND m.days_of_supply > 0 THEN m.dispense_date + (m.days_of_supply - 1) ELSE m.dispense_date END AS drug_exposure_end_date,
        m.dispense_qty,
        m.days_of_supply,
        mv.visit_occurrence_id,
        REPLACE(SUBSTR(COALESCE(NULLIF(TRIM(m.drug_ndc_normalized), ''), m.drug_name), 1, 50), '*', '') AS drug_source_value,
        COALESCE(m.drug_source_concept_id, 0) AS drug_source_concept_id,
        ROW_NUMBER() OVER (PARTITION BY m.drug_exposure_id ORDER BY m.order_id, m.dispense_date, m.fill_no) AS rn
    FROM mapped m
